...
Then make the posts index the home page in [`config/routes.rb`](https://github.com/arkenidar/wsite/blob/main/config/routes.rb):
...
- [`db/migrate/20260621212802_create_posts.rb`](https://github.com/arkenidar/wsite/blob/main/db/migrate/20260621212802_create_posts.rb) — creates the `posts` table; `change` is reversible, `t.timestamps` adds `created_at`/`updated_at`.
- [`db/schema.rb`](https://github.com/arkenidar/wsite/blob/main/db/schema.rb) — the authoritative snapshot of the DB structure (generated; never edit by hand).
- [`app/models/post.rb`](https://github.com/arkenidar/wsite/blob/main/app/models/post.rb) — the model; empty here because the DB defines the shape ("convention over configuration").
- [`app/controllers/posts_controller.rb`](https://github.com/arkenidar/wsite/blob/main/app/controllers/posts_controller.rb) — the 7 actions; serves both HTML and JSON.
- [`app/views/posts/`](https://github.com/arkenidar/wsite/blob/main/app/views/posts/) — `index/show/new/edit` pages, plus `_form` and `_post` **partials**, and `.jbuilder` JSON views.
- [`config/routes.rb`](https://github.com/arkenidar/wsite/blob/main/config/routes.rb) — `resources :posts` and `root`.
