...
1. **Generate the job** and make it do something observable —
   [`app/jobs/notify_author_job.rb`](https://github.com/arkenidar/wsite/blob/main/app/jobs/notify_author_job.rb):
...
3. **Use Solid Queue in development** (it's already the production default). This app
   ...
   - [`config/database.yml`](https://github.com/arkenidar/wsite/blob/main/config/database.yml) — adds a `queue` sub-database under
     `development` (`storage/development_queue.sqlite3`, `migrations_paths: db/queue_migrate`).
   - [`config/environments/development.rb`](https://github.com/arkenidar/wsite/blob/main/config/environments/development.rb):
   ...
   - Create the queue DB from its schema ([`db/queue_schema.rb`](https://github.com/arkenidar/wsite/blob/main/db/queue_schema.rb)):
...
- In production this app can run the worker *inside* Puma via `SOLID_QUEUE_IN_PUMA: true`
  (see [`config/deploy.yml`](https://github.com/arkenidar/wsite/blob/main/config/deploy.yml)), so there's no separate worker process
  to deploy. Scheduled (cron-like) jobs live in [`config/recurring.yml`](https://github.com/arkenidar/wsite/blob/main/config/recurring.yml).
...
[`test/jobs/notify_author_job_test.rb`](https://github.com/arkenidar/wsite/blob/main/test/jobs/notify_author_job_test.rb):
