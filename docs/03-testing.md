...
|-------|-------|-------|---------|
| **Model** | fastest | validations, associations — no HTTP | [`test/models/post_test.rb`](../test/models/post_test.rb), [`test/models/comment_test.rb`](../test/models/comment_test.rb) |
| **Integration** | fast | full route → controller → view over HTTP, no browser | [`test/controllers/posts_controller_test.rb`](../test/controllers/posts_controller_test.rb) |
| **System** | slow | a real headless browser via Capybara | [`test/system/posts_test.rb`](../test/system/posts_test.rb) |
...
in [`test/fixtures/posts.yml`](../test/fixtures/posts.yml) and referenced as `posts(:one)`.
