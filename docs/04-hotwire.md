...
1. **Nest the routes** in [`config/routes.rb`](../config/routes.rb):
...
3. **Stream templates** describe the DOM changes:
   - [`app/views/comments/create.turbo_stream.erb`](../app/views/comments/create.turbo_stream.erb):
   ...
   - [`app/views/comments/destroy.turbo_stream.erb`](../app/views/comments/destroy.turbo_stream.erb):
...
4. **Views**: the comments list + form live on the post show page
   ([`app/views/posts/show.html.erb`](../app/views/posts/show.html.erb)) with a
...
5. **Stimulus sprinkle** — a live character counter,
   [`app/javascript/controllers/character_counter_controller.js`](../app/javascript/controllers/character_counter_controller.js),
   wired into the post body field in [`app/views/posts/_form.html.erb`](../app/views/posts/_form.html.erb)
   via data attributes:
...
- No **jQuery**, no **Webpacker/Node**. JavaScript is loaded through **importmap**
  ([`config/importmap.rb`](../config/importmap.rb)); Stimulus controllers auto-register
  by filename via [`app/javascript/controllers/index.js`](../app/javascript/controllers/index.js).
...
- Tests: [`test/system/comments_test.rb`](../test/system/comments_test.rb) asserts the
